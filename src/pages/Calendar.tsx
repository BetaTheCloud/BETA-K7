...
export default function Calendar() {
  ...
  const getTypeIcon = (type: CalendarEvent['type']) => {
    switch (type) {
      ...
      default: return <CheckCircle2 strokeWidth={1.5} className="w-5 h-5 text-amber-500 dark:text-amber-400" />;
    }
  };
      ...
      <header className="mb-8 border-b border-[#e6e2d6] dark:border-teal-700/50 pb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="text-3xl font-display font-extrabold text-stone-900 dark:text-white flex items-center gap-3 tracking-tight">
            <div className="p-2.5 rounded-xl bg-amber-50 dark:bg-[#1d3540]mber-500/10 border border-amber-200/50 dark:border-amber-500/20">
      ...
      <div className="space-y-10">
        {Object.entries(groupedEvents).map(([term, termEvents]: [string, CalendarEvent[]]) => (
          <div key={term} className="space-y-6">
            <h3 className="text-xl font-display font-bold text-stone-900 dark:text-white border-b border-[#e6e2d6]/60 dark:border-teal-700/40 pb-2 flex items-center gap-2">
            ...
            <div className="relative pl-6 before:content-[''] before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-stone-100 dark:before:bg-teal-700/40 space-y-6">
              {termEvents.map((event) => (
                <div key={event.id} className="relative group">
                  ...
                  <div className="bg-[#fcfbf9] dark:bg-[#264653] border border-[#e6e2d6] dark:border-teal-700/50 rounded-2xl p-5 ml-4 shadow-sm hover:shadow-md transition-shadow">
