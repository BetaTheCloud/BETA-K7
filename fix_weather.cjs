...
const widgetCode = `import { useState, useEffect } from 'react';
import { Cloud, CloudDrizzle, CloudFog, CloudLightning, CloudRain, CloudSnow, CloudSun, Moon, Sun, Loader2, Droplets, Wind } from 'lucide-react';
...
export default function WeatherWidget() {
  ...
  useEffect(() => {
    async function fetchWeather() {
      try {
        const res = await fetch('https://api.open-meteo.com/v1/forecast?latitude=36.7161&longitude=37.1150&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m,is_day&timezone=Europe%2FIstanbul');
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center w-48 h-16 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 animate-pulse">
  ...
  const getWeatherInfo = (code: number, isDay: number) => {
    switch (true) {
      ...
      case (code >= 71 && code <= 77) || (code >= 85 && code <= 86):
        return { text: 'Kar Yağışlı', Icon: CloudSnow, color: 'text-sky-200' };
      case code >= 95:
  ...
  return (
    <div className="flex items-center gap-4 bg-white/10 backdrop-blur-md px-5 py-3 rounded-2xl border border-white/10 shadow-lg">
      <div className={\`p-2.5 rounded-xl bg-black/20 shadow-inner \${info.color}\`}>
      ...
      <div className="flex flex-col justify-center">
        <div className="flex items-baseline gap-2">
          <div className="flex items-start">
            ...
            <span className="text-sm font-medium text-white/70 mt-1 ml-0.5">°C</span>
          </div>
        ...
        <div className="flex items-center gap-3 mt-0.5 text-xs font-medium text-white/60">
...
`;
fs.writeFileSync('src/components/WeatherWidget.tsx', widgetCode);
